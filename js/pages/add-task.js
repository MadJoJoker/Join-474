...
import { currentPriority, setMedium, initPriorityButtons } from '../events/priorety-handler.js';
// KORRIGIERTER IMPORTPFAD
import { selectedCategory, selectedContacts, initDropdowns, clearCategory } from '../events/dropdown-menu.js';
...
export function openPicker() {
    if (picker) {
        ...
        console.error("Flatpickr-Instanz 'picker' ist nicht initialisiert. Stellen Sie sicher, dass initAddTaskForm() aufgerufen wurde.");
    }
}
...
function checkRequiredFields() {
    ...
    if (!selectedCategory) { // Verwendet selectedCategory aus dropdown-menu.js
    ...
    if (selectedContacts.length === 0) { // Verwendet selectedContacts aus dropdown-menu.js
...
function createTaskObject() {
    ...
    return {
        ...
        description,
        dueDate,
        category: selectedCategory, // Verwendet selectedCategory aus dropdown-menu.js
        priority: currentPriority, // Verwendet currentPriority aus priority-handler.js
        assignedTo: selectedContacts.map(contact => ({ // Verwendet selectedContacts aus dropdown-menu.js
            name: contact.name,
...
export async function handleCreateTask(event) {
    ...
    if (checkRequiredFields()) {
        ...
        if (overlay) {
            ...
            initAddTaskForm(); // Initialisiert das Formular neu, was auch die Dropdowns und Kontakte neu einrichtet
        }
    } else {
...
export async function initAddTaskForm() {
    ...
    document.getElementById('subtasks-list')?.addEventListener('click', (even) => {
